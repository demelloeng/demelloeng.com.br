...
import {nodes,routes,starts,keyOf,titleOf,isProject,valid,toggle,updateAnswer,switchRoute,advance,routeNodes,summary,suggestedServices,safetyHold,missingData,ufs,areaZeroValid,HOURLY} from './journey.mjs';
...
export function App(){
 ...
 function sendCase(payload){setSendState('sending');submitToCRM(payload,hp).then(r=>{setSendResult(r);setSendState(r.ok?'sent':'error');});}
 ...
 {id==='X4'&&submitted&&sendState==='sent'&&<div className="result-card success" role="status"><Check size={36}/><h2>Recebemos as informações do seu caso.</h2><p>A DEMELLO vai analisar e poderá entrar em contato pelos dados que você informou. Você pode baixar um resumo legível para guardar; as fotos não são enviadas.</p><p>{NEXT_STEP}</p><button type="button" className="secondary-button" onClick={download}><DownloadSimple size={20}/>Baixar resumo do seu caso</button></div>}
 {id==='X4'&&submitted&&sendState==='error'&&<div className="result-card needs-review" role="alert"><h2>Não foi possível enviar agora.</h2><p>Sua previsão e o resumo continuam disponíveis. Tente enviar novamente ou fale com a DEMELLO pelos canais do site.</p><div className="actions"><button type="button" className="secondary-button" onClick={()=>sendCase(sentPayload.current)}>Tentar enviar novamente</button><button type="button" className="secondary-button" onClick={download}><DownloadSimple size={20}/>Baixar resumo do seu caso</button></div></div>}
 </form>
