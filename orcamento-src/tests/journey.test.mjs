...
test('Construção, ampliação e indefinição convergem sem novas perguntas',()=>{const construction=walk('build');assert.deepEqual(construction.path,['HOME','CA1','CA_AREA','CA2','CA3','CA4','CA5','CA6','CA7','Q_FUND','CA8','X1']);assert.deepEqual(walk('build',{CA1:'Ampliar um imóvel existente'}).path,['HOME','CA1','CA_EXISTING','CA_NEW','CA2','CA3','CA4','CA5','CA6','CA7','Q_FUND','CA8','X1']);assert.ok(!walk('build',{CA1:'Ainda estou definindo'}).path.some(x=>['CA_AREA','CA_NEW','CA_EXISTING'].includes(x)));});
...
test('Disciplinas sugeridas precisam de confirmação no próprio A7',()=>{let a={route:'build',property:'Casa/sobrado',services:['Não sei quais preciso']};assert.equal(valid('CA7',a),false);a={...a,confirmedSuggestions:suggestedServices(a),suggestionConfirmed:true};assert.ok(valid('CA7',a));assert.equal(advance('CA7',a).id,'Q_FUND'); /* estrutural sugerido e confirmado => pergunta a área de projeção */ assert.equal(advance('Q_FUND',{...a,area_fundacao:{value:'80',unknown:false}}).id,'CA8');assert.equal(valid('CA7',updateAnswer(a,'CA5',{value:'4'})),false);assert.deepEqual(toggle(['Estrutural'],'Não sei quais preciso','Não sei quais preciso'),['Não sei quais preciso']);});
test('Vários serviços seguem juntos por uma única coleta',()=>{const {a,path}=walk('known',{S1:['Estrutural','Drenagem','Hidrossanitário']});assert.deepEqual(path,['HOME','S1','Q_FUND','S2','S3','S4','S5','S6','X1']);assert.deepEqual(a.services,['Estrutural','Drenagem','Hidrossanitário']);});
